function symbolic_discretize(pdesys::ModelingToolkit.PDESystem,discretization::DiffEqOperators.MOLFiniteDifference)
    ...
    if length(nottime) == 1
        ...
        derivars = [[LinearAlgebra.dot(left_weights(j),depvar[left_idxs]), LinearAlgebra.dot(right_weights(j),depvar[right_idxs(j)])]
    ...
    #bceqs = isempty(bceqs) ? [] : reduce(vcat,bceqs)
    ...
    # Finalize
    defaults = pdesys.ps === nothing || pdesys.ps === SciMLBase.NullParameters() ? u0 : vcat(u0,pdesys.defaults...)
...
function discretize(pdesys::ModelingToolkit.PDESystem,discretization::DiffEqOperators.MOLFiniteDifference)
    sys, tspan = symbolic_discretize(pdesys,discretization)
    simpsys = ModelingToolkit.structural_simplify(sys)
